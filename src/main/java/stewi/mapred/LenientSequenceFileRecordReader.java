...
public class LenientSequenceFileRecordReader<K, V> implements RecordReader<K, V> {

  private LenientSequenceFile.Reader in;
  private long start;
  private long end;
  private boolean more = true;
  protected Configuration conf;

  public LenientSequenceFileRecordReader(Configuration conf, FileSplit split)
    throws IOException {
    Path path = split.getPath();
    FileSystem fs = path.getFileSystem(conf);
    this.in = new LenientSequenceFile.Reader(fs, path, conf);
